...
volatile float Step_Input = 450;                // Step Input PWM output (0-255)
...
// old Kp (lab 2): 0.0635
// 0.02773998321
static float Kp = 0.04334372377;                // Set Proportioanl Gain     
static float Ki = 0;                // Set Integral Gain

// old Kd (lab 2): 0.00365
// 0.0025374
static float Kd = 0.0031818;                // Set Derivative Gain

// old N (lab 2): 958.1013
// 790.0763
static float N = 1200; // TODO
...
// 15.9
static unsigned int Period = 12.7; // TODO
// 0.0159 -> 2
...
{  
  ...
  {  
    ...
    // go into PD controller mode
    if (Received ==1) {
      volatile long int position_offset = Received_Position+8;
      ...
      while (DONE != 1){
        while (Timer_Go == 1){
          ...
          CLStep(position_offset);  
          Move_Motor(Controller_Output);
          //as instructed by Chris
          while (Timer_Go == 0) {}
          Timer_Go = 0;
        }
      }
